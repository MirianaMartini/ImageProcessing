...
def find_gesture_module(unknownGesture, knownGestures, keyPoints, gestNames, tol):
    ...
    # the real time detected gesture
    for i in range(0, len(gestNames), 1):
        error = find_error_module(knownGestures[i], unknownGesture, keyPoints)
        errorArray.append(error)
    errorMin = errorArray[0]
    minIndex = 0
...
def main():
    ...
    gestNames, knownGestures = get_known_gestures("GesturesFilesModules/")

    while cap.isOpened():
        # get the current frame
        frame, pTime = grab_frame_module(cap, detector, gestNames, knownGestures, pTime)
